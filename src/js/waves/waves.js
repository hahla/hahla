...
require('ng-cache!../../partials/waveg.html');
// var s = require('./shared.js');
// var simulation = require('./simulation.js');
// var ui = require('./ui.js');

var wavesController = function() {

    // shared.js_start
    var INITIAL_SIZE = 250,
        INITIAL_WIND = [10.0, 10.0],
        INITIAL_CHOPPINESS = 6.5;
...
//     var init = function() {
//         if (s.hasWebGLSupportWithExtensions(['OES_texture_float', 'OES_texture_float_linear'])) {
//             main();
//         } else {
//             document.getElementById('error').style.display = 'block';
//             document.getElementById('footer').style.display = 'none';
...
//     var main = function () {
//         var simulatorCanvas = document.getElementById(s.SIMULATOR_CANVAS_ID),
//             overlayDiv = document.getElementById(s.OVERLAY_DIV_ID),
//             uiDiv = document.getElementById(s.UI_DIV_ID),
//             cameraDiv = document.getElementById(s.CAMERA_DIV_ID),
//             windDiv = document.getElementById(s.WIND_SPEED_DIV_ID),
//             windSpeedSpan = document.getElementById(s.WIND_SPEED_SPAN_ID),
//             choppinessDiv = document.getElementById(s.CHOPPINESS_DIV_ID),
//             sizeSpan = document.getElementById('size-value');

//         s.setText(choppinessDiv, s.INITIAL_CHOPPINESS, s.CHOPPINESS_DECIMAL_PLACES);
//         s.setText(sizeSpan, s.INITIAL_SIZE, s.SIZE_DECIMAL_PLACES);

//         var camera = new simulation.Camera(),
//             projectionMatrix = s.makePerspectiveMatrix(new Float32Array(16), s.FOV, s.MIN_ASPECT,
//                 s.NEAR, s.FAR);

//         var simulator = new simulation.Simulator(simulatorCanvas, window.innerWidth, window.innerHeight);

//         var profile = new ui.Profile(document.getElementById(s.PROFILE_CANVAS_ID)),
//             sizeSlider = new ui.Slider(cameraDiv, s.SIZE_SLIDER_X, s.SIZE_SLIDER_Z,
//                 s.SIZE_SLIDER_LENGTH, s.MIN_SIZE, s.MAX_SIZE, s.INITIAL_SIZE,
//                 s.SIZE_SLIDER_BREADTH, s.SIZE_HANDLE_SIZE),
//             choppinessSlider = new ui.Slider(cameraDiv, s.CHOPPINESS_SLIDER_X, s.CHOPPINESS_SLIDER_Z,
//                 s.CHOPPINESS_SLIDER_LENGTH, s.MIN_CHOPPINESS, s.MAX_CHOPPINESS, s.INITIAL_CHOPPINESS,
//                 s.CHOPPINESS_SLIDER_BREADTH, s.CHOPPINESS_HANDLE_SIZE);
...
//         var setUIPerspective = function (height) {
//             var fovValue = 0.5 / Math.tan(s.FOV / 2) * height;
//             s.setPerspective(uiDiv, fovValue + 'px');
//         };

//         var windArrow = new ui.Arrow(cameraDiv, s.INITIAL_WIND[0], s.INITIAL_WIND[1]);
//         s.setText(windSpeedSpan, windArrow.getValue(), s.WIND_SPEED_DECIMAL_PLACES);
//         s.setTransform(windDiv, 'translate3d(' + s.WIND_SPEED_X + 'px, 0px, ' + Math.max(s.MIN_WIND_SPEED_Z, windArrow.getTipZ() + s.WIND_SPEED_OFFSET) + 'px) rotateX(90deg)');

//         var inverseProjectionViewMatrix = [],
//             nearPoint = [],
//             farPoint = [];
//         var unproject = function (viewMatrix, x, y, width, height) {
//             s.premultiplyMatrix(inverseProjectionViewMatrix, viewMatrix, projectionMatrix);
//             s.invertMatrix(inverseProjectionViewMatrix, inverseProjectionViewMatrix);

//             s.setVector4(nearPoint, (x / width) * 2.0 - 1.0, ((height - y) / height) * 2.0 - 1.0, 1.0, 1.0);
//             s.transformVectorByMatrix(nearPoint, nearPoint, inverseProjectionViewMatrix);

//             s.setVector4(farPoint, (x / width) * 2.0 - 1.0, ((height - y) / height) * 2.0 - 1.0, -1.0, 1.0);
//             s.transformVectorByMatrix(farPoint, farPoint, inverseProjectionViewMatrix);

//             s.projectVector4(nearPoint, nearPoint);
//             s.projectVector4(farPoint, farPoint);
...
//             var mousePosition = s.getMousePosition(event, uiDiv);
//             var mouseX = mousePosition.x,
//                 mouseY = mousePosition.y;

//             var point = unproject(camera.getViewMatrix(), mouseX, mouseY, width, height);

//             if (windArrow.distanceToTip(point) < s.ARROW_TIP_RADIUS) {
//                 mode = s.ROTATING;
//             } else if (sizeSlider.distanceToHandle(point) < s.SIZE_HANDLE_RADIUS) {
//                 mode = SLIDING_SIZE;
//             } else if (choppinessSlider.distanceToHandle(point) < s.CHOPPINESS_HANDLE_RADIUS) {
//                 mode = s.SLIDING_CHOPPINESS;
//             } else {
//                 mode = s.ORBITING;
//                 lastMouseX = mouseX;
//                 lastMouseY = mouseY;
...
//             if (windArrow.distanceToTip(point) < s.ARROW_TIP_RADIUS || mode === s.ROTATING) {
//                 overlayDiv.style.cursor = 'move';
//             } else if (sizeSlider.distanceToHandle(point) < s.SIZE_HANDLE_RADIUS ||
//                 choppinessSlider.distanceToHandle(point) < s.CHOPPINESS_HANDLE_RADIUS ||
//                 mode === s.SLIDING_SIZE || mode === s.SLIDING_CHOPPINESS) {
//                 overlayDiv.style.cursor = 'ew-resize';
//             } else if (mode === s.ORBITING) {
//                 overlayDiv.style.cursor = '-webkit-grabbing';
//                 overlayDiv.style.cursor = '-moz-grabbing';
//                 overlayDiv.style.cursor = 'grabbing';
//             } else {
...
//             if (mode === s.ORBITING) {
//                 camera.changeAzimuth((mouseX - lastMouseX) / width * s.SENSITIVITY);
//                 camera.changeElevation((mouseY - lastMouseY) / height * s.SENSITIVITY);
//                 lastMouseX = mouseX;
//                 lastMouseY = mouseY;
//             } else if (mode === s.ROTATING) {
//                 windArrow.update(point[0], point[2]);
//                 simulator.setWind(windArrow.getValueX(), windArrow.getValueY());
//                 s.setText(windSpeedSpan, windArrow.getValue(), s.WIND_SPEED_DECIMAL_PLACES);

//                 s.setTransform(windDiv, 'translate3d(' + s.WIND_SPEED_X + 'px, 0px, ' + Math.max(s.MIN_WIND_SPEED_Z, windArrow.getTipZ() + s.WIND_SPEED_OFFSET) + 'px) rotateX(90deg)');
//             } else if (mode === s.SLIDING_SIZE) {
//                 sizeSlider.update(point[0], function (size) {
//                     simulator.setSize(size);
//                     s.setText(sizeSpan, size, s.SIZE_DECIMAL_PLACES);
//                 });
//             } else if (mode === s.SLIDING_CHOPPINESS) {
//                 choppinessSlider.update(point[0], function (choppiness) {
//                     simulator.setChoppiness(choppiness);
//                     s.setText(choppinessDiv, choppiness, s.CHOPPINESS_DECIMAL_PLACES);
//                     profile.render(choppiness);
//                 });
//             }
//         });
...
//             if (windowWidth / windowHeight > s.MIN_ASPECT) {
//                 s.makePerspectiveMatrix(projectionMatrix, s.FOV, windowWidth / windowHeight, s.NEAR, s.FAR);
//                 simulator.resize(windowWidth, windowHeight);
//                 uiDiv.style.width = windowWidth + 'px';
//                 uiDiv.style.height = windowHeight + 'px';
//                 cameraDiv.style.width = windowWidth + 'px';
...
//                 height = windowHeight;
//             } else {
//                 var newHeight = windowWidth / s.MIN_ASPECT;
//                 s.makePerspectiveMatrix(projectionMatrix, s.FOV, windowWidth / newHeight, s.NEAR, s.FAR);
//                 simulator.resize(windowWidth, newHeight);
//                 simulatorCanvas.style.top = (windowHeight - newHeight) * 0.5 + 'px';
//                 uiDiv.style.top = (windowHeight - newHeight) * 0.5 + 'px';
//                 setUIPerspective(newHeight);
...
//             var fovValue = 0.5 / Math.tan(s.FOV / 2) * height;
//             s.setTransform(cameraDiv, 'translate3d(0px, 0px, ' + fovValue + 'px) ' + s.toCSSMatrix(camera.getViewMatrix()) + ' translate3d(' + width / 2 + 'px, ' + height / 2 + 'px, 0px)');
//             simulator.render(deltaTime, projectionMatrix, camera.getViewMatrix(), camera.getPosition());
